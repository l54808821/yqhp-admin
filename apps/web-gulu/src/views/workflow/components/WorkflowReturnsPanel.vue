...
const localList = ref<WorkflowReturn[]>([...(props.returns || [])]);

watch(
  () => props.returns,
  (newReturns) => {
    const incoming = [...(newReturns || [])];
...
function handleFieldChange(index: number, field: keyof WorkflowReturn, value: any) {
...
<template>
  <div class="returns-panel">
    <div v-if="localList.length === 0" class="empty-row">
      <span class="empty-text">暂无返回值</span>
