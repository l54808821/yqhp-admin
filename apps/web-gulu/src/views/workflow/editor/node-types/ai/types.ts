...
export const builtinTools: BuiltinTool[] = [
  ...
  // 联网工具
  { name: 'web_search', label: '联网搜索', description: '搜索互联网信息，支持中英文，返回结构化搜索结果', category: 'web' },
  { name: 'web_read', label: '网页读取', description: '读取网页内容并返回干净的 Markdown 格式，自动处理 JS 渲染', category: 'web' },
  // 代码执行
...
export function createDefaultAIConfig(): AIConfig {
  return {
    ...
    timeout: 300,
    tools: ['web_search', 'web_read', 'code_execute'],
    mcp_server_ids: [],
    skill_ids: [],
    max_tool_rounds: 15,
    knowledge_base_ids: [],
