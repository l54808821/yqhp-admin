...
import type { DeepAgentConfig } from '../shared/types';
import { createDefaultDeepAgentConfig } from '../shared/types';
...
const emit = defineEmits<{
  (e: 'update', node: DeepAgentStepNode): void;
}>();

const localNode = ref<DeepAgentStepNode | null>(null);
  ...
  (newNode) => {
    if (newNode) {
      ...
      if (!localNode.value!.config) {
        localNode.value!.config = createDefaultDeepAgentConfig();
      }
    }
  },
...
function handleConfigUpdate(patch: Partial<DeepAgentConfig>) {
...
function handleRun() {
  ...
  run({
    ...
    type: 'ai_deep_agent',
    name: localNode.value.name || '深度代理',
            ...
            <Form layout="vertical">
              <Form.Item label="系统提示词">
                <Input.TextArea
                  ...
                  :rows="4"
                  placeholder="可选：设定主 Agent 的角色和行为..."
                  @change="
                    (e: any) =>
          ...
          <Tabs.TabPane key="sub-agents" tab="子 Agent">
            <Form layout="vertical">
              <Form.Item label="子 Agent 节点 ID">
                ...
                />
                <div class="param-hint">
                  输入工作流中其他 AI 节点的 ID，用逗号分隔。主 Agent
                  通过规划将子任务分配给这些子 Agent，上下文相互隔离。
                </div>
              </Form.Item>
            </Form>
          </Tabs.TabPane>
