...
<template>
  <Drawer
    ...
    :width="500"
    :destroyOnClose="true"
    :body-style="{ padding: '16px 20px', overflowY: 'auto' }"
    @close="visible = false"
  >
    <Form layout="vertical">
      ...
      <template v-if="form.type === 'graph'">
        <Form.Item label="实体抽取模型" style="margin-top: 12px">
          <Select
      ...
      <div class="form-row">
        <Form.Item label="默认检索数量 (Top-K)" class="form-item-half">
          <InputNumber
        ...
        <Form.Item label="相似度阈值" class="form-item-half">
          <InputNumber
